    ...
    uint8_t **pLout, uint32_t *pLlen, int upd_seq_no)
{
  ...
  status = hdr & LHDCV5_DEC_HDR_LATENCY_MASK;

  //Get number of frame in packet.
  status = (hdr & LHDCV5_DEC_HDR_FRAME_NO_MASK) >> 2;
...
int32_t lhdcv5BT_dec_init_decoder(HANDLE_LHDCV5_BT *handle, tLHDCV5_DEC_CONFIG *config)
{
  ...
  ALOGD("%s: bits_depth:%u sample_rate=%u bit_rate=%u version=%d lossless_enable=%d lossless_raw_enable=%d", __func__,
      config->bits_depth, config->sample_rate, config->bit_rate, config->version,
      config->lossless_enable, config->lossless_raw_enable);
  ...
  if ((config->lossless_raw_enable != 0) &&
      (config->lossless_raw_enable != 1)) {
    ALOGD("%s: lossless raw enable invalid value %d ", __func__, config->lossless_raw_enable);
    return LHDCV5BT_DEC_API_INVALID_INPUT;
  }

  if (config->lossless_raw_enable == 1 && config->lossless_enable == 0) {
    ALOGD("%s: disable lossless raw mode because lossless disable", __func__);
    config->lossless_raw_enable = 0;
  }

  lhdcv5_util_dec_register_log_cb(&print_log_cb, log_buff, LHDCV5_DEC_LOG_LEVEL_DEBUG);
  ...
  func_ret = lhdcv5_util_init_decoder(hLhdcBT, config->bits_depth,
      config->sample_rate, config->bit_rate, config->lossless_enable,
      config->lossless_raw_enable, config->version);
  ...
  *handle = hLhdcBT;
  ...
  func_ret = lhdcv5_util_dec_channel_selsect(LHDCV5_OUTPUT_STEREO);
  if (func_ret != LHDCV5_UTIL_DEC_SUCCESS) {
    ...
    uint32_t frameBytes, uint32_t *packetBytes)
{
  ...
  lhdcv5_frame_Info_t lhdc_frame_Info;
    ...
    uint8_t *pcmData, uint32_t *pcmBytes, uint32_t bits_depth)
{
  ...
  *pcmBytes = 0;
  ...
  ALOGD("%s: output frame samples %d", __func__, (int)frame_samples);
  ...
  while ((frame_num > 0) && (ptr_offset < in_len))
  {
    ...
    ALOGD("%s: frame_num[%d]: decoded success. input_frame_len %d output_len %d", __func__,
