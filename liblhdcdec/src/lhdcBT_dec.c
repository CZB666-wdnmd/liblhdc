...
int lhdcBT_dec_init_decoder(tLHDCV3_DEC_CONFIG *config)
{
  uint32_t mem_req_bytes = 0;
    ...
    //ALOGD("%s: bits_depth:%d sample_rate=%d version=%d", __func__,	config->bits_depth, config->sample_rate, config->version);
  ...
  lhdc_register_log_cb(&print_log_cb, log_buff, LHDC_DEC_LOG_LEVEL_DEBUG);

  lhdc_util_dec_get_mem_req(config->version, &mem_req_bytes);
  if (mem_req_bytes <= 0)
  {
    return LHDCBT_DEC_FUNC_FAIL;
  }

  hLhdcBT = malloc(mem_req_bytes);
  if (hLhdcBT == NULL)
  {
    return LHDCBT_DEC_FUNC_FAIL;
  }

  ALOGD(" %s: start lhdcInit..", __func__);
  lhdcInit(hLhdcBT, config->bits_depth, config->sample_rate, 400000, config->version);
...
int lhdcBT_dec_check_frame_data_enough(const uint8_t *frameData, uint32_t frameBytes, uint32_t *packetBytes)
{
	...
	*packetBytes = 0;
	...
	ALOGD(" %s: in_buf (%p), frameData (%p), in_len (%d), frame_num (%d)", __func__, in_buf, frameData, (int)in_len, (int) frame_num);
	...
	while ((frame_num > 0) && (ptr_offset < in_len))
	{
		...
		if (fn_ret == false)
		{
			ALOGD(" %s: lhdcFetchFrameInfo(%d) fail..", __func__, (int)frame_num);
			return LHDCBT_DEC_FUNC_FAIL;
		}
		
		ALOGD(" %s: lhdcFetchFrameInfo  frame_num (%d), ptr_offset (%d), lhdc_frame_Info.frame_len (%d), in_len (%d)", __func__, (int)frame_num, (int)ptr_offset, (int)lhdc_frame_Info.frame_len, (int)in_len);
		
		if ((ptr_offset + lhdc_frame_Info.frame_len) > in_len)
		{
			ALOGD(" %s: Not Enough... frame_num(%d), ptr_offset(%d), frame_len(%d), in_len (%d)", __func__, (int)frame_num, (int)ptr_offset, (int)lhdc_frame_Info.frame_len, (int)in_len);
...
int lhdcBT_dec_decode(const uint8_t *frameData, uint32_t frameBytes, uint8_t* pcmData, uint32_t* pcmBytes, uint32_t bits_depth)
{
    ...
    //ALOGD(" %s: enter, assemble_lhdc_packet(frameBytes %d)", __func__, (int)frameBytes);
...
*/
	...
	//ALOGD(" %s: frameData=0x%p, in_buf=0x%p, in_len=%d", __func__, frameData, in_buf, in_len);
    //ALOGD(" %s: get frame_num=%d", __func__, (int)frame_num);
	...
	while ((frame_num > 0) && (ptr_offset < in_len))
	{
        ...
        //ALOGD(" %s: lhdcDecodeProcess(frm=%d, frame_len=%d out_len=%d)..", __func__, (int)frame_num, (int)lhdc_frame_Info.frame_len, (int)lhdc_out_len);
    ...
    *pcmBytes = (uint32_t) dec_sum;
    ...
    //ALOGD(" %s: end (dec_sum=%d pcmBytes=%d)", __func__, (int)dec_sum, (int)*pcmBytes);
    return LHDCBT_DEC_FUNC_SUCCEED;
}
...
static int assemble_lhdc_packet(uint8_t *input, uint32_t input_len, uint8_t **pLout, uint32_t *pLlen, int upd_seq_no)
{
    ...
    status = hdr & LHDC_DEC_HDR_LATENCY_MASK;

    //Get number of frame in packet.
    status = (hdr & LHDC_DEC_HDR_FRAME_NO_MASK) >> 2;
    ...
    *pLout = input;
    ...
    ALOGD(" %s: end frame number (%d)", __func__, ret);
    return ret;
}
