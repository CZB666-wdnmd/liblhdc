...
// enable for support lossless raw mode
#define LHDCV5BT_LOSSLESS_RAW_SUPPORT
...
#define ABR_MAX_STAGE_BITRATE             400 // max bitrate(kbps) (NOTE: must be smaller and equal to LHDCV5_ABR_MAX_BITRATE)
...
// this ABR table is used when lossless-24bit enabled.
static uint32_t auto_bitrate_adjust_table_lhdcv5_lless24_48k[] = {128, 192, 256, 320, 400, ABR_MAX_STAGE_BITRATE};
static uint32_t auto_bitrate_adjust_table_lhdcv5_lless24_96k[] = {256, 320, 400, 400, 400, ABR_MAX_STAGE_BITRATE};
...
#define VBR_UP_LOSSY_RATIO_THRESHOLD      0     // (default) lossy ratio threshold(percentage: 0~100)
#define VBR_DOWN_LOSSLESS_RATIO_THRESHOLD 100   // (default) lossless ratio threshold(percentage: 0~100)
...
#define LLESS_RECORD_TICKS               (4500) // define total number ticks to a long-term statistics (by tick count).
#endif
/*******************************************************************************/
...
static int32_t lhdcv5_enc_lossless_dump_statis(lhdcv5_abr_para_t * lless_para)
{
  ...
  if (lless_para->lless_stat_showBitrateCnt % LLESS_SHOW_BITRATE_TICKS == 0)
  {
    ...
    ALOGD ("[LLESS_DBG][VBR_ADJ],statisK_BRate,%04u,%03u,%03u,dn:[%u/%u],up:[%u/%u],"
        "TH:[%u,%u],INTV:[%u,%u],statCnt:[%u/%u],finalFlag:[%u]",
        ...
        LLESS_RECORD_TICKS,
        lless_para->lless_stat_bitrateRec_done);
    ...
    {
      ALOGD ("[LLESS_DBG][VBR_ADJ] final time's up! set flag");
...
{
  if (bitrate_inx == NULL || abr_table == NULL)
  {
...
#ifdef LHDCV5BT_LOSSLESS_RAW_SUPPORT
...
)
{
  ...
  {
    vbr_type = LHDCV5_VBR_48K_RES;
  }
  else if (handle_vbr->sample_rate == LHDCV5_SR_96000HZ)
  {
    vbr_type = LHDCV5_VBR_96K_RES;
  }
  else
  {
    func_ret = LHDCV5_FRET_ERROR;
    ALOGE ("%s: not supported sample rate (%d)", __func__,
        handle_vbr->sample_rate);
    goto fail;
  }

  func_ret = lhdcv5_util_vbr_plus_process(handle, vbr_type);
  if (func_ret != LHDCV5_FRET_SUCCESS)
  {
    ALOGE ("%s: lhdcv5_util_vbr_plus_process error %d", __func__, func_ret);
    goto fail;
  }
...
)
{
  ...
  if (handle_vbr->sample_rate != LHDCV5_SR_48000HZ &&
      handle_vbr->sample_rate != LHDCV5_SR_96000HZ)
  {
  ...
  if (handle_vbr->bits_per_sample != LHDCV5BT_SMPL_FMT_S16 &&
      handle_vbr->bits_per_sample != LHDCV5BT_SMPL_FMT_S24)
  {
  ...
  {
    vbr_type = LHDCV5_VBR_48K_RES;
    if (handle_vbr->bits_per_sample == LHDCV5BT_SMPL_FMT_S24)
    {
      abr_table = &(auto_bitrate_adjust_table_lhdcv5_lless24_48k[0]);
    }
    else
    {
  ...
  else if (handle_vbr->sample_rate == LHDCV5_SR_96000HZ)
  {
    ...
    if (handle_vbr->bits_per_sample == LHDCV5BT_SMPL_FMT_S24)
    {
      abr_table = &(auto_bitrate_adjust_table_lhdcv5_lless24_96k[0]);
    }
    else
    {
      ALOGE ("%s: not support bit per sample (%d)", __func__, handle_vbr->bits_per_sample);
      return LHDCV5_FRET_INVALID_HANDLE_PARA;
    }
  }
  else
  {
    ALOGE ("%s: not support sample rate (%d)", __func__, handle_vbr->sample_rate);
    return LHDCV5_FRET_INVALID_HANDLE_PARA;
  }
  ...
  if (handle_vbr->dnBitrateCnt > 0 && handle_vbr->dnBitrateCnt >= ABR_DOWN_RATE_TIME_CNT)
  {
    ...
    if (queueLength > DEMOTE_TO_ABR_QLENGTH_THRESHOLD)
    {
      ...
      // DEMOTE -> turn OFF lossless
      func_ret = lhdcv5_util_set_lossless_status(handle, LHDCV5_LLESS_OFF);
      if (func_ret != LHDCV5_FRET_SUCCESS)
      {
        ALOGW ("[AUTO_BITRATE][VBR_ADJ](DEMOTE) lhdcv5_util_set_lossless_status error %d", func_ret);
        goto fail;
      }

      ALOGD ("[AUTO_BITRATE][VBR_ADJ](DEMOTE) bitrate(%u)[%u] to bitrate(%u)[%u] queueLength(%u) lless_on(%u)",
          handle_vbr->lastBitrate, last_bitrate_inx,
          abr_table[vbr_demote_bitrate_inx], new_bitrate_inx,
          queueLength,
          handle_vbr->is_lless_on);
...
) 
{
  ...
  uint32_t abr_to_vbr_promote_bitrate = 0;
  uint32_t new_bitrate_inx = 0;
  uint32_t new_bitrate_inx_set = 0;
  uint32_t last_bitrate_inx = 0;
  ...
  else if (handle_abr->sample_rate == LHDCV5_SR_48000HZ)
  {
    ...
    if (handle_abr->is_lless_enabled == LHDCV5_LLESS_ENABLED &&
        handle_abr->bits_per_sample == LHDCV5BT_SMPL_FMT_S24)
    {
      abr_table = &(auto_bitrate_adjust_table_lhdcv5_lless24_48k[0]);
    }
    else
  ...
  if (handle_abr->bits_per_sample == LHDCV5BT_SMPL_FMT_S16)
  {
    abr_to_vbr_promote_bitrate = LHDCV5_VBR_PROMOTE_BITRATE;
  }
  else if (handle_abr->bits_per_sample == LHDCV5BT_SMPL_FMT_S24)
  {
    abr_to_vbr_promote_bitrate = LHDCV5_VBR_LLES24_PROMOTE_BITRATE;
  }
  ...
  if (handle_abr->dnBitrateCnt > 0 && handle_abr->dnBitrateCnt >= ABR_DOWN_RATE_TIME_CNT)
  {
    ...
    if (queueLength > ABR_DOWN_QUEUE_LENGTH_THRESHOLD)
    {
          ...
          new_abr_bitrate_inx < gABR_table_index)
      {
        ...
        ALOGD ("[AUTO_BITRATE][ABR_ADJ](DN) bitrate(%u)[%u] to bitrate(%u)[%u], queueLength(%u)",
  ...
  if (handle_abr->upBitrateCnt > 0 && handle_abr->upBitrateCnt >= ABR_UP_RATE_TIME_CNT)
  {
    ...
    {
      ALOGV ("[AUTO_BITRATE][ABR_ADJ](UP) current queuSumTmp:%u", queuSumTmp);
          ...
          (new_abr_bitrate_inx > gABR_table_index))
      {
        ...
        ALOGD ("[AUTO_BITRATE][ABR_ADJ](UP) bitrate(%u)[%u] to bitrate(%u)[%u], queuSumTmp(%u)",
            abr_table[gABR_table_index], gABR_table_index,
            abr_table[new_abr_bitrate_inx], new_abr_bitrate_inx,
            queuSumTmp);
        func_ret = lhdcv5_util_reset_down_bitrate (handle);
      ...
      else
      {
            ...
            handle_abr->lastBitrate == ABR_MAX_STAGE_BITRATE)
        {
          ...
          func_ret = lhdcv5_util_get_bitrate_inx (abr_to_vbr_promote_bitrate, &new_bitrate_inx);
          if (func_ret != LHDCV5_FRET_SUCCESS)
          {
            ALOGW ("[AUTO_BITRATE][ABR_ADJ](PROMOTE) lhdcv5_util_get_bitrate_inx error %d", func_ret);
            goto fail;
          ...
          // PROMOTE -> turn ON lossless
          func_ret = lhdcv5_util_set_lossless_status(handle, LHDCV5_LLESS_ON);
          if (func_ret != LHDCV5_FRET_SUCCESS)
          {
            ALOGW ("[AUTO_BITRATE][VBR_ADJ](PROMOTE) lhdcv5_util_set_lossless_status error %d", func_ret);
            goto fail;
          }

          ALOGD ("[AUTO_BITRATE][ABR_ADJ](PROMOTE) bitrate(%u)[%u] to bitrate(%u)[%u], queuSumTmp(%u) lless_on(%u)",
              abr_table[last_abr_bitrate_inx], last_bitrate_inx,
              abr_to_vbr_promote_bitrate, new_bitrate_inx_set,
              queuSumTmp,
              handle_abr->is_lless_on);
        }
        else
        {
...
) 
{
  ...
  lhdcv5_util_enc_register_log_cb(&print_log_cb, log_buff, LHDCV5_LOG_LEVEL_DEBUG);
...
)
{
  ...
  if (enabled != LHDCV5_LLESS_RAW_ENABLED && enabled != LHDCV5_LLESS_RAW_DISABLED) {
    ALOGW ("%s: invalid input!", __func__);
    return LHDCV5_FRET_INVALID_HANDLE_CB;
  }
  ...
  if (func_ret != LHDCV5_FRET_SUCCESS)
  {
    ALOGW ("%s: Failed to set lossless raw mode (%d)!", __func__, func_ret);
    return LHDCV5_FRET_ERROR;
  }

  ALOGD ("%s: set lossless raw mode (%u)!", __func__, enabled);
...
{
  ALOGD ("%s: feature not support!", __func__);
  return LHDCV5_FRET_SUCCESS;
}
#endif
...
) 
{
  ...
  ALOGD ("%s: get bitrate (%u)!", __func__, *bitrate);
...
) 
{
  ...
  case LHDCV5_ENC_TYPE_LHDCV5:
    ...
    if (abr_para->is_lless_enabled != LHDCV5_LLESS_ENABLED)
    {
      // ABR mode (lossy mode) only
      ALOGD ("%s: lossy ABR: (current bitrate: %u)...", __func__, abr_para->lastBitrate);
      func_ret = lhdcv5_enc_abr_adjust_bitrate (handle, abr_para, queueLen);
    }
    else if (abr_para->is_lless_enabled == LHDCV5_LLESS_ENABLED &&
        (abr_para->sample_rate == LHDCV5_SR_48000HZ ||
            abr_para->sample_rate == LHDCV5_SR_96000HZ) &&
        (abr_para->bits_per_sample == LHDCV5BT_SMPL_FMT_S16 ||
            abr_para->bits_per_sample == LHDCV5BT_SMPL_FMT_S24))
    {
#ifdef LHDCV5BT_LOSSLESS_RAW_SUPPORT
      if (abr_para->is_lless_raw == LHDCV5_LLESS_RAW_ENABLED)
      {
          if (abr_para->is_lless_on == LHDCV5_LLESS_ON)
          {
            // lossless raw mode: VBR-plus: (lossless raw VBR-plus)
            ALOGD ("%s: VBR-Plus: (current bitrate: %u)...", __func__, abr_para->lastBitrate);
            func_ret = lhdcv5_enc_vbr_plus_adjust_bitrate(handle, abr_para);
          }
          else
          {
            ALOGD ("%s: VBR-Plus: (Error) lossless not ON!", __func__);
            return LHDCV5_FRET_ERROR;
          }
      }
      else
#endif
      {
        // std lossless mode: VBR hybrid mode(lossy ABR / lossless VBR)
        if (abr_para->is_lless_on == LHDCV5_LLESS_ON)
        {
          // run in VBR layer
          ALOGD ("%s: VBR hybrid: go VBR (current bitrate: %u)...", __func__, abr_para->lastBitrate);
          func_ret = lhdcv5_enc_vbr_adjust_bitrate (handle, abr_para, queueLen);
        }
        else
        {
          // run in ABR layer
            ALOGD ("%s: VBR hybrid: go ABR (current bitrate: %u)...", __func__, abr_para->lastBitrate);
          func_ret = lhdcv5_enc_abr_adjust_bitrate (handle, abr_para, queueLen);
        }
      }
    } else {
      ALOGW ("%s: Non supported auto bit rate mode!", __func__);
      return LHDCV5_FRET_ERROR;
    }
...
//		enabled: ext. function is set to enable(true) or disable(false)
//		priv: a pointer to the exra data needed for ext. function
//		priv_data_len: number of bytes of the extra data
//	Return
//		LHDCV5_FRET_SUCCESS: succeed to set specified ext. function to enable(true) or disable(false)
//		Other: fail to set .
//----------------------------------------------------------------
